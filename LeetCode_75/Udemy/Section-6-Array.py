...
def find_pairs(nums: List[int], target: int) -> None:
...
def find_number(array: List[int], number: int) -> None:
...
# Best Score
def first_second(my_list: List[int]) -> Tuple[int, int]:
...
# Pairs
def pair_sum(arr: List[int], target_sum: int) -> List[str]:
    result: List[str] = []
...
def permuntation(list1: List[int], list2: List[int]) -> bool:
